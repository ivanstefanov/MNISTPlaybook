...
def standardize(Xtr, Xva):
  ...
  return (Xtr - mean) / std, (Xva - mean) / std, mean, std
...
def main():
  ...
  parser.add_argument("--model_dir", type=str, default="/mnt/model")
  ...
  # 1) Създаваме папка за артефакти (за сървинг)
  os.makedirs(args.model_dir, exist_ok=True)

  # 2) Запазваме preprocessing параметрите (важно!)
  torch.save(
  {"mean": mean.cpu(), "std": std.cpu()},
  os.path.join(args.model_dir, "preprocess.pt")
  )

  # 3) Експортираме модела като TorchScript (serving-friendly)
  net.eval()
  example = torch.randn(1, in_features, dtype=torch.float32)
  traced = torch.jit.trace(net, example)
  traced.save(os.path.join(args.model_dir, "model.pt"))

  print(f"model_saved={os.path.join(args.model_dir, 'model.pt')}", flush=True)
  print(f"preprocess_saved={os.path.join(args.model_dir, 'preprocess.pt')}", flush=True)
